 ...
 You should use a unique directory for each training run, or use 'force=True' to automatically clear previous monitor files.'''.format(
    ...
    def _after_step(self, observation, reward, done, info):
        ...
        if done and self.env_semantics_autoreset:
            # For environments with BlockingReset wrapping VNCEnv, this observation will be the first one of the new episode
            self._reset_video_recorder()
            self.episode_id += 1
            self._flush()

        if info.get('true_reward',
                    None):  # Semisupervised environments modify the rewards, but we want the original when scoring
